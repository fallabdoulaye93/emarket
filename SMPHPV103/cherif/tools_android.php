...
function Connection()
{
    ...
    $conn = NULL;
    try {
        $conn = new PDO("mysql:host=85.236.155.92;dbname=admincpanel20_el2021_db", "admincpanel20_userdb2021", "[ZHfp(ygoZUJ&");
...
function login($login, $password){
    try{
        $dbh = Connection();
        //var_dump($dbh);die();
        $sql = "SELECT u.*, pf.id as profileid, pf.libelle, pf.etat as profiletat FROM sf_user u INNER JOIN sf_profil pf ON pf.id = u.sf_profil_id WHERE u.login = :login AND password = :mypass";
        ...
        if($stmt->rowCount() === 1){
            ...
            if($user->etat == '1'){
                return json_encode(array('errorCode' => 0, 'errorMessage' => $user));
            }
            else{
